...
class CAMarksWebScrapper:
    ...
    def fetch_ca_marks(self):
        ...
        if table2 is None:
            raise NoCAMarksAvailable
        return CAMarksWebScrapper.parse_table_data_as_ca_marks_models(
            self.parse_table(table1)
        ),CAMarksWebScrapper.parse_table_data_as_ca_marks_models(
            self.parse_table(table2)
        )
